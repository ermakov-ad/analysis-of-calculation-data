...
txt_file_name_table = "Re_Ro_table.txt"
...
def get_data_from_file(time, xinterp, yinterp, zinterp, x, y, z):
    vel = readvector(path, str(time), 'U')
    print("end reading velocity in time " + str(time))

    u = griddata((x, y, z), vel[0, :], (xinterp, yinterp, zinterp), method='nearest')
    v = griddata((x, y, z), vel[1, :], (xinterp, yinterp, zinterp), method='nearest')
    w = griddata((x, y, z), vel[2, :], (xinterp, yinterp, zinterp), method='nearest')
    del vel
    collect()
    return u, v, w
...
def find_energy_spectrum(u, v, w, size_of_array, count_of_cells, freq_z, freq_y, freq_x, amplitude, max_wave_deviation_):
    ...
    k_array = np.linspace(0.0, amplitude, num=count_of_cells)
...
kx1 = np.arange(0.0, (cells_number_axis // 2 - count_of_boundary_cells) / 2.0, 0.5, dtype=float) + 0.5
kx = np.concatenate([kx1, np.flip(-1.0*kx1, 0)])
ky1 = np.arange(0.0, (cells_number_axis // 2 - count_of_boundary_cells) / 2.0, 0.5, dtype=float) + 0.5
ky = np.concatenate([ky1, np.flip(-1.0*ky1, 0)])
kz1 = np.arange(0.0, (cells_number_axis // 2 - count_of_boundary_cells) / 2.0, 0.5, dtype=float) + 0.5
kz = np.concatenate([kz1, np.flip(-1.0*kz1, 0)])
#kx = np.arange(cells_number_axis // 2) / 2.0
#ky = np.arange(cells_number_axis // 2) / 2.0
#kz = np.arange(cells_number_axis // 2) / 2.0
#kx = np.arange(cells_number_axis // 2)
#ky = np.arange(cells_number_axis // 2)
#kz = np.arange(cells_number_axis // 2)

#kx_3d_array = np.multiply(np.ones((cells_number_axis // 2, cells_number_axis // 2, cells_number_axis // 2)), kz)
#ky_3d_array = np.multiply(np.ones((cells_number_axis // 2, cells_number_axis // 2, cells_number_axis // 2)), ky.reshape((cells_number_axis // 2, 1)))
#kz_3d_array = np.multiply(np.ones((cells_number_axis // 2, cells_number_axis // 2, cells_number_axis // 2)), kx.reshape((cells_number_axis // 2, 1, 1)))
kx_3d_array = np.multiply(np.ones((cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells)), kz)
ky_3d_array = np.multiply(np.ones((cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells)), ky.reshape((cells_number_axis - 2*count_of_boundary_cells, 1)))
kz_3d_array = np.multiply(np.ones((cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells, cells_number_axis - 2*count_of_boundary_cells)), kx.reshape((cells_number_axis - 2*count_of_boundary_cells, 1, 1)))
...
amplitude_wave_vec = np.linalg.norm(np.array([cells_number_axis / 4.0, cells_number_axis / 4.0, cells_number_axis / 4.0]))
...
def find_inertial_wave_from_files(file_number_array):
    ...
    fourier_u = fourier_u[: l//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
    fourier_v = fourier_v[: l//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
    fourier_w = fourier_w[: l//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
...
def find_inertial_wave_from_arrays(number_of_files, u_center_arr, v_center_arr, w_center_arr):
    ...
    fourier_u = fourier_u[: number_of_files//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
    fourier_v = fourier_v[: number_of_files//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
    fourier_w = fourier_w[: number_of_files//2, : cells_number_axis//2, : cells_number_axis//2, : cells_number_axis//2]
...
def find_energy_spectrum_from_file(file_number, count_of_vec_points, amplitude_wave_vec, kx, ky, kz):
    ...
    E_i = find_energy_spectrum(fourier_u, fourier_v, fourier_w, cells_number_axis // 2, count_of_vec_points, kz, ky, kx, amplitude_wave_vec, max_wave_deviation_in_energy_calc)
...
def find_spectrums_and_ampl_of_inertial_waves_from_file(start_time, end_time, time_step, bool_save_spectrum, count_of_vec_steps, count_of_theta, delta_theta, bool_save_txt_info):
    ...
    for i in np.arange(start_time, end_time + time_step, time_step):
        ...
        energy_sp = find_energy_spectrum(fourier_u, fourier_v, fourier_w, cells_number_axis // 2, count_of_vec_steps, kz, ky, kx, amplitude_wave_vec, max_wave_deviation_in_energy_calc)
        ...
        if bool_save_spectrum == True:
            ...
            save_spectral_density_of_the_dissipation_rate(amplitude_wave_vec, max_wave_deviation_in_dencity_velocity_dissipation_calc, count_of_vec_steps, dencity_velocity_sp, start_time + i*time_step)
        if bool_save_txt_info == True:
...
def find_spectrums_and_characteristic_numbers_from_files(start_time, end_time, time_step, count_of_vec_steps, bool_save_txt_info, bool_save_spectrum):
    ...
    xinterp, yinterp, zinterp = create_structured_mesh(cells_number_axis)
    x, y, z = readmesh(path)
    ...
    for i in np.arange(start_time, end_time + time_step, time_step):
        if (i % 1 == 0):
            i = int(i)
        print('time = ' + str(i))
        u_center, v_center, w_center = get_data_from_file(i, xinterp, yinterp, zinterp, x, y, z)
        ...
        u_center_new = np.copy(u_center[count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells])
        v_center_new = np.copy(v_center[count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells])
        w_center_new = np.copy(w_center[count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells, count_of_boundary_cells : cells_number_axis - count_of_boundary_cells])
        ...
        fourier_u, fourier_v, fourier_w = find_fourier_velocity_vec(u_center_new, v_center_new, w_center_new)
        fig, axs = plt.subplots(2, 3)
        axs[0, 0].imshow(u_center_new[cells_number_axis // 2])
        axs[0, 1].imshow(v_center_new[cells_number_axis // 2])
        axs[0, 2].imshow(w_center_new[cells_number_axis // 2])
        axs[1, 0].imshow(fourier_u[cells_number_axis // 2])
        axs[1, 1].imshow(fourier_v[cells_number_axis // 2])
        axs[1, 2].imshow(fourier_w[cells_number_axis // 2])

        axs[0, 0].set_title('u')
        axs[0, 1].set_title('v')
        axs[0, 2].set_title('w')
        axs[1, 0].set_title('FFT(u)')
        axs[1, 1].set_title('FFT(v)')
        axs[1, 2].set_title('FFT(w)')
        plt.show()
        energy_sp = find_energy_spectrum(fourier_u, fourier_v, fourier_w, cells_number_axis - 2*count_of_boundary_cells, count_of_vec_steps, kz, ky, kx, amplitude_wave_vec, max_wave_deviation_in_energy_calc)
        ...
        q_max = (np.argmax(energy_sp_copy)) * amplitude_wave_vec / count_of_vec_steps
        full_e = find_full_energy(u_center_new, v_center_new, w_center_new)

        print("time moment: " + str(i))
        print("full kinetic energy = " + str(full_e))
        print("maximum spectrum of energy in wave vector = " + str(q_max))
